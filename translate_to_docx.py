...
# Title
title = doc.add_heading('BrewXOS Small Smart Fermentation System', level=0)
title.alignment = WD_ALIGN_PARAGRAPH.CENTER
sub = doc.add_paragraph()
sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = sub.add_run('Hardware Recommendations')
run.bold = True
run.font.size = Pt(16)
...
p.add_run('Sub-50 cm Cube Prototype | Brewer\'s Spent Grain Solid-Liquid Fermentation & Enzymatic Hydrolysis Monitoring | iGEM Hardware Track\n').italic = True
p2 = doc.add_paragraph()
p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
p2.add_run('Author: Thomas ZHAO, Hardware Lead, Shanghai Hongwen School Pudong iGEM 2026 Team').italic = True
...
add_h1('I. Overall Assessment')
p = doc.add_paragraph()
p.add_run('Conclusion: ').bold = True
p.add_run('Build a ')
p.add_run('desktop-style smart fermentation monitoring system').italic = True
p.add_run(' suitable for iGEM exhibition and small-scale experiments. We do not promise industrial-grade fully automatic sterilization, fully automatic cleaning, or real-time on-line quantification of AXOS structural specificity. Within a 50 cm cube envelope, a more realistic target is a ')
p.add_run('1-2 L semi-automatic fermentation system').bold = True
p.add_run(' with controllable temperature, agitation, pH / DO / temperature logging, periodic sampling, and colorimetric estimation of reducing-sugar / oligosaccharide release trends - used as a ')
p.add_run('process-control indicator').italic = True
p.add_run(' for feeding, extending, or terminating the batch.')

p = doc.add_paragraph()
p.add_run('Recommended positioning: ').bold = True
p.add_run('BrewXOS-Fermenter Mini').bold = True
p.add_run(' - a small intelligent solid-liquid fermentation / enzymatic hydrolysis rig. It is ')
p.add_run('not').italic = True
p.add_run(' an industrial production device. It is a project-validation, education-exhibition, and process-parameter exploration platform.')

# II. Recommended Prototype Form
add_h1('II. Recommended Prototype Form')
...
data = [
    ...
    ('Base controller', 'ESP32 or Arduino Mega + touch screen / buttons + SD card / serial data export; Raspberry Pi option for graphical UI.'),
...
# III. Function Feasibility
add_h1('III. Function Feasibility & Implementation Plan')
...
# IV. AXOS Colorimetric Detection
add_h1('IV. AXOS Colorimetric Detection Module Recommendations')
p = doc.add_paragraph()
p.add_run('Do not claim ').add_text = ' '
p.add_run('"on-line quantification of AXOS"').italic = True
p.add_run('. A more accurate statement is: ')
p.add_run('"intermittent colorimetric estimation of reducing-sugar / oligosaccharide release trends, used as a process-control indicator"').italic = True
p.add_run('. The DNS method is commonly used for reducing-sugar quantification, with absorbance read near 540 nm - but in complex sugar mixtures, the readout is not one-to-one with specific AXOS content.')

p = doc.add_paragraph()
p.add_run('Recommended control logic: ').bold = True
p.add_run('Build a D-xylose or XOS standard curve, and use the ')
p.add_run('rate of change').italic = True
p.add_run(' of colorimetric readings - not absolute AXOS content - to judge the fermentation process. For example, if two consecutive readings grow below threshold and pH / DO indicate metabolism slowing, the system should suggest "consider feeding / extending enzymatic hydrolysis / ending the batch".')

# V. Suggested Additional Functions
add_h1('V. Suggested Additional Functions')
bullets = [
...
for title_txt, body in bullets:
    p = doc.add_paragraph(style='List Bullet')
    p.add_run(title_txt).bold = True
    p.add_run(body)

# VI. Prototype Version Recommendations
add_h1('VI. Prototype Version Recommendations')
...
# VII. Cost Estimation
add_h1('VII. Cost Estimation (Sub-50 cm Cube Prototype)')
p = doc.add_paragraph('The figures below are educational / prototype-level estimates; prices vary with sourcing channel, domestic vs. imported, new vs. used, and required custom fabrication. DO probes and custom mechanical parts are the most likely budget overruns.')
...
data = [
    ...
    ('Control & display', '150-500', '500-1500', 'ESP32 / Arduino, screen, SD card, PSU, buttons; Raspberry Pi raises cost.'),
...
p = doc.add_paragraph()
p.add_run('Overall budget guidance: ').bold = True
p.add_run('V1 (low-end) ~ RMB 2,500-6,000; V2 (recommended prototype) ~ RMB 7,000-15,000; V3 (research-grade) ~ RMB 15,000-30,000+. If a high-quality optical DO probe is selected and significant custom mechanical work is needed, the budget will rise noticeably.')

# VIII. Implementation Timeline
add_h1('VIII. Recommended Implementation Timeline')
timeline = [
...
for title_txt, body in timeline:
    p = doc.add_paragraph(style='List Bullet')
    p.add_run(title_txt).bold = True
    p.add_run(body)

# IX. Risks
add_h1('IX. Risks & Statement Boundaries')
risks = [
...
# X. Hardware Highlights
add_h1('X. Hardware Highlights for Wiki / Presentation')
highlights = [
    ('"Black-box cultivation" -> Visible, logged, decision-supportable workflow.', 'Transform BrewXOS\'s fermentation process from '),
]
p = doc.add_paragraph(style='List Bullet')
p.add_run('Transform BrewXOS\'s fermentation process from ').add_text = ' '
p.add_run('"black-box cultivation"').italic = True
p.add_run(' into a ')
p.add_run('visible, logged, and decision-supportable').bold = True
p.add_run(' workflow.')

p = doc.add_paragraph(style='List Bullet')
p.add_run('Use pH, DO, temperature, and colorimetric trends to drive ')
p.add_run('feed / batch-end decisions').bold = True
p.add_run(' - instead of ending batches blindly on a fixed time schedule.')

p = doc.add_paragraph(style='List Bullet')
p.add_run('Engineer ')
p.add_run('anti-clog sampling and low-speed high-torque agitation').bold = True
p.add_run(' tailored specifically to the BSG solid-liquid system - a project-specific design.')

p = doc.add_paragraph(style='List Bullet')
p.add_run('Combine ')
p.add_run('waste valorization + synthetic biology + engineering control').bold = True
p.add_run(' into a single device well-suited for ')
p.add_run('Education / Human Practices exhibition').bold = True
p.add_run('.')

# References
add_h1('References')
p = doc.add_paragraph()
p.add_run('Pricing references: ').bold = True
p.add_run('DFRobot Arduino pH kit publicly listed at ~USD 29.50; DFRobot analog DO kit publicly listed at ~USD 169; some optical DO sensor vendors quote USD 280-360 class. ')
p.add_run('Detection reference: ').bold = True
p.add_run('DNS reducing-sugar method is typically read near 540 nm, but in complex sugar mixtures the readout requires a standard curve and careful interpretation.')
...
# Save
output_path = r'C:\Users\xiaomi\Desktop\igem\BrewXOS_Small_Smart_Fermentation_System_Hardware_Recommendations.docx'

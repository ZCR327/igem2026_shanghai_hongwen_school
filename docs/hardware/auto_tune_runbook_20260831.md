...
| Ki | 0.05 | 保守值 |
| Kd | 3.0 | 优化: 8.0 → 3.0（防过冲 — **有帕尔贴后 Kd 不用太激进**） |
| target | 35°C | 优化: 37 → 35（XOS 净积累多 30-50%） |

**5 路继电器触屏控制（8.31 老师确认）**：

| 字符 | PIN | 硬件 | 验证方法 |
|------|-----|------|---------|
| `c` / `d` | 41 | 蠕动泵 A | 触摸听声音 |
| `e` / `f` | 42 | 蠕动泵 B | 触摸听声音 |
| `a` / `b` | 43 | 加热膜（主加热，PID 也用） | 触摸 a 后看 DS18B20 上升 |
| `g` | 44 | **副加热继电器** | 触摸 g 后温度上升加快 |
| `h` | 45 | **TEC 帕尔贴（制冷）**| 触摸 h 后温度下降 |
| `i` | 44+45 | 组合键（一键关副加热+制冷）| 同时听不到任何动作 |
